...
const Canvas = () => {
  ...
  const handleCaptureCanvas = async () => {
    ...
    try {
      ...
      console.log(`Exporting ${shapeIds.length} shapes at scale ${scale.toFixed(2)}`);
      ...
      console.log("Capture successful. Base64 Size:", (result.url.length / 1024).toFixed(2), "KB");

      setSnapshot(result.url);
      setOpenConvert(true);
    } catch (error) {
      console.error("Capture Error:", error);
      alert(error instanceof Error ? error.message : "Failed to capture sketch.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="h-[calc(100vh-65px)] overflow-auto p-1 relative">
      <div className="absolute z-20 top-2 left-1/2 -translate-x-1/2 flex items-center gap-10">
        ...
        >
          {loading ? "Capturing..." : "Convert to Code"} <Brain className="ml-2 size-4" />
        </Button>
      </div>
      ...
      <Dialog open={openConvert} onOpenChange={setOpenConvert}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            {snapshot && (
              <div className="border rounded-md overflow-hidden bg-gray-50 flex justify-center p-2">
                <img src={snapshot} alt="Sketch Preview" className="max-h-40 object-contain" />
              </div>
            )}
          ...
          <DialogFooter className="mt-4">
            <Button variant="ghost" onClick={() => setOpenConvert(false)} disabled={loading}>
              Cancel
            </Button>

            <Button
              disabled={loading}
              onClick={handleSendToVision}
            >
